...
CREATE TABLE IF NOT EXISTS TagValues (projectId VARCHAR(36), exampleId VARCHAR(36), tagName VARCHAR(36), tagValue VARCHAR(36), FOREIGN KEY (projectId, exampleId, tagName) REFERENCES TagNames(projectId, exampleId, tagName), PRIMARY KEY(projectId, exampleId, tagName));
CREATE TABLE IF NOT EXISTS Logs (id VARCHAR(36) PRIMARY KEY, userId VARCHAR(36), projectId VARCHAR(36), exampleId VARCHAR(36), actionType VARCHAR(50), details VARCHAR(255), timeStamp DATETIME, FOREIGN KEY (userId) REFERENCES Users(id), FOREIGN KEY (exampleId) REFERENCES Examples(id), FOREIGN KEY (projectId) REFERENCES Projects(id));